package cn.bugfish.dove_wz25.UserMannageSystem.Service;
...
public class UserService {
    ...
    public boolean registerUser(UserBean user) throws SQLException {
        ...
        try {
            ...
            // 定义 SQL 语句，用于插入新用户数据
            String sql = "INSERT INTO users (steam64id, qqnumber, email, password) VALUES (?,?,?,?)";
            // 创建 PreparedStatement 对象，用于执行 SQL 语句
            PreparedStatement stmt = conn.prepareStatement(sql);
            // 设置 SQL 语句中的参数
            stmt.setString(1, user.getSteam64id());
            stmt.setString(2, user.getQqnumber());
